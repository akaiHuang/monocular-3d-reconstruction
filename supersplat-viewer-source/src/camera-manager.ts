...
const tmpv = new Vec3();
const defaultFrameDirection = new Vec3(2, 1, 2);
...
const calcFrameDistance = (bbox: BoundingBox, fov: number) => {
    const sceneSize = bbox.halfExtents.length();
    if (sceneSize === 0) {
        return 0;
    }
    return sceneSize / Math.sin(fov / 180 * Math.PI * 0.5);
};

const resolveFrameDirection = (position: Vec3, target: Vec3) => {
    const direction = new Vec3().sub2(position, target);
    if (direction.lengthSq() < 1e-6) {
        direction.copy(defaultFrameDirection);
    }
    return direction.normalize();
};

const createFrameCamera = (bbox: BoundingBox, fov: number, direction: Vec3) => {
    const distance = calcFrameDistance(bbox, fov);
    const frameDirection = direction.clone().normalize();
    return createCamera(
        frameDirection.mulScalar(distance).add(bbox.center),
        bbox.center,
        fov
    );
};
...
class CameraManager {
    ...
    constructor(global: Global, bbox: BoundingBox) {
        ...
        const basePosition = new Vec3(camera0.position);
        const baseTarget = new Vec3(camera0.target);
        const frameDirection = resolveFrameDirection(basePosition, baseTarget);
        const frameCamera = createFrameCamera(bbox, camera0.fov, frameDirection);

        const boundRadius = bbox.halfExtents.length();
        const targetDistance = tmpv.copy(baseTarget).sub(bbox.center).length();
        const shouldAutoCenter = boundRadius > 0 && targetDistance > boundRadius * 0.5;

        let resetTarget = baseTarget.clone();
        let resetPosition = basePosition.clone();

        if (shouldAutoCenter) {
            resetTarget = bbox.center.clone();
            const offset = resetTarget.clone().sub(baseTarget);
            resetPosition.add(offset);
        }

        const minDistance = calcFrameDistance(bbox, camera0.fov);
        if (shouldAutoCenter && minDistance > 0) {
            const currentDistance = resetPosition.distance(resetTarget);
            if (currentDistance < minDistance * 0.9) {
                resetPosition = frameDirection.clone().mulScalar(minDistance).add(resetTarget);
            }
        }

        const resetCamera = createCamera(resetPosition, resetTarget, camera0.fov);
